      ...
      */
       ...
       */

       {
           Rect rect; 
           /*
           * поля инициализировались значением -858993460. компилятор msvc создает такой маркер чтоб в режиме отладки показать что человек забыл инициализировать переменную
           * на строчке Rect rect будет вызван неявный конструктор по умолчанию ( конструктор который компилятор создает автоматически для класса если нет написанного конструктора )
           * после закрытия фигурной скобки вызовется неявный деструктор ( опять же деструктор который компилятор генеритрует сам если нет написанного )
           * 
           * в этом случае конструктор и деструктор не делают ничего
           */
           std::cout << "Rect создан";
       } 
